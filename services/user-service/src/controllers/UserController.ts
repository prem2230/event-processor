...
class UserController {
  ...
  public static async register(
    ...
    try {
      return res.status(201).json(await UserController.userService.register(req.body));
    } catch (error) {
      const code =
  ...
  public static async verifyCredentials(
    ...
    const profile = await UserController.userService.verifyCredentials(req.body);
    return profile
  ...
  public static async getProfile(
    ...
    const profile = await UserController.userService.getProfile(userId);
    return profile
      ? res.status(200).json(profile)
